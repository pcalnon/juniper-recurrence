...
Memory Unit discretisation (C1-clean, irregular-Δt-native). This package ships both the
Δt-native memory unit and — now that the shared ``juniper-model-core`` package has landed —
:class:`FixedOrderLMURegressor`, the recurrent model implementing that package's
``TrainableModel`` interface (a fixed memory with a closed-form least-squares readout), plus
its :class:`LMURegressorSerializer`.
