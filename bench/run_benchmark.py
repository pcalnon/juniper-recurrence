...
def evaluate_bands(results: dict[str, dict[str, Any]]) -> list[dict[str, Any]]:
    """Evaluate the ratified OQ-14 acceptance bands against the collected results."""
    ...
    # Band 1 — Δt thesis: var-Δt LMU >= 25% lower RMSE than fixed-Δt on irregular_sine.
    if "irregular_sine" in results:
        fr, vr = rmse("irregular_sine", fixed), rmse("irregular_sine", var)
        improvement = (fr - vr) / fr if fr > 0 else 0.0
        bands.append({"band": "1 — Δt thesis (irregular_sine: var-Δt ≥25% lower RMSE than fixed-Δt)", "value": f"{improvement:+.1%} RMSE reduction (fixed={fr:.4f} → var={vr:.4f})", "pass": improvement >= 0.25})
    ...
    for name in results:
        ...
        bands.append({"band": f"2 — {name}: LMU beats naive & matches/beats linear (r2)", "value": f"LMU={r2(name, var):.4f}  naive={r2(name, 'naive_persistence'):.4f}  linear={r2(name, 'linear_ridge'):.4f}", "pass": bool(beats_naive and matches_linear)})
    ...
    for name, res in results.items():
        if res["grid"] == "regular":
            ...
            bands.append({"band": f"3 — {name} (regular): var-Δt ≈ fixed-Δt (≤10% RMSE gap)", "value": f"{penalty:.1%} gap (fixed={fr:.4f}, var={vr:.4f})", "pass": penalty <= 0.10})

    return bands


def _render_report(results: dict[str, dict[str, Any]], bands: list[dict[str, Any]]) -> str:
    lines = ["# Recurrence Benchmark — Results", "", "Generated by `python -m bench.run_benchmark` (reproducible from the dataset seeds).", "", "## Per-dataset metrics (walk-forward CV; mean ± std across folds)", ""]
    for name, res in results.items():
        ...
        lines.append("")
    lines.append("## Acceptance bands (OQ-14)")
    lines.append("")
    ...
    lines.append("")
    overall = "PASS" if all(b["pass"] for b in bands) else "PARTIAL — see misses above"
...
def main() -> None:
    ...
    report = _render_report(results, bands)
    (_RESULTS / "REPORT.md").write_text(report)
    print("\n" + report)
    print(f"[bench] wrote {_RESULTS}/REPORT.md + per-dataset JSON")
